...
var _default = {
  ...
  methods: (_methods = {
    // 缓存上一次的设备状态
    cacheDeviceState: function cacheDeviceState() {
      var state = {
        mode: this.mode,
        angle: this.angle,
    ...
    // 恢复缓存的状态
    restoreDeviceState: function restoreDeviceState() {
      var state = uni.getStorageSync('deviceState');
      if (state) {
    ...
    getDataFromOnenet: function getDataFromOnenet() {
      ...
      return (0, _asyncToGenerator2.default)( /*#__PURE__*/_regenerator.default.mark(function _callee() {
        var isManualRefresh, retryCount, maxRetries, result;
        return _regenerator.default.wrap(function _callee$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                isManualRefresh = _arguments.length > 0 && _arguments[0] !== undefined ? _arguments[0] : false;
                retryCount = 0;
                maxRetries = 3;
              case 3:
  ...
  }, (0, _defineProperty2.default)(_methods, "getDataFromOnenet", function getDataFromOnenet() {
    var _this5 = this;
    ...
    return new Promise(function (resolve, reject) {
      uni.request({
        ...
        header: {
          'authorization': _this5.token // 认证token
        },
        ...
        success: function success(res) {
          if (res.data && res.data.code === 0) {
            ...
            // 根据标识符更新本地数据
            _this5.light = dataMap.light; // 光照强度
            _this5.angle = dataMap.angle; // 当前角度

            // 模式更新需要特殊处理，避免覆盖本地更改
            var serverMode = parseInt(dataMap.mode);

            // 如果开启了本地模式覆盖，检查时间戳
            if (_this5.localModeOverride) {
              var now = Date.now();
              // 如果本地模式切换后的5秒内，忽略服务器模式值
              if (now - _this5.lastModeChangeTime < 5000) {
                console.log('忽略服务器模式更新，保持本地模式:', _this5.mode);
              } else {
                // 5秒后恢复正常更新
                _this5.localModeOverride = false;
                _this5.mode = serverMode;
              }
            } else {
              // 没有本地覆盖时正常更新
              _this5.mode = serverMode;
            ...
            if (!_this5.isEditingTime) {
              // 只有当用户没有在编辑时才更新时间值
              if (dataMap.start_time) _this5.start_time = dataMap.start_time;
              if (dataMap.end_time) _this5.end_time = dataMap.end_time;
            } else {
              console.log('用户正在编辑时间，跳过服务器数据更新');
            }

            // 只有在非编辑状态下才更新角度值，防止滑块位置跳变
            if (!_this5.isEditingTime && dataMap.timer_angle !== undefined) {
              _this5.timer_angle = dataMap.timer_angle;
            }
            resolve(res.data);
          } else {
            var _res$data;
            _this5.errorMsg = '获取数据失败：' + (((_res$data = res.data) === null || _res$data === void 0 ? void 0 : _res$data.msg) || '未知错误');
            reject(new Error(_this5.errorMsg));
          }
        },
        fail: function fail(err) {
          _this5.errorMsg = '网络请求失败，请检查网络连接';
  ...
  }), (0, _defineProperty2.default)(_methods, "triggerUpdateAnimation", function triggerUpdateAnimation() {
    var _this6 = this;
    this.dataUpdated = true;
    setTimeout(function () {
      _this6.dataUpdated = false;
    }, 1000);
  }), (0, _defineProperty2.default)(_methods, "retryConnection", function retryConnection() {
    this.errorMsg = '';
    this.getDataFromOnenet();
  }), (0, _defineProperty2.default)(_methods, "sendPropertyUpdate", function sendPropertyUpdate(params) {
    var _this7 = this;
  ...
  }), (0, _defineProperty2.default)(_methods, "onCurtainModeSwitch", function onCurtainModeSwitch(event) {
    var _this8 = this;
  ...
  }), (0, _defineProperty2.default)(_methods, "onCurtainForward", function onCurtainForward() {
    // 确保 this.angle 是数字类型
  ...
  }), (0, _defineProperty2.default)(_methods, "onCurtainReverse", function onCurtainReverse() {
    // 确保 this.angle 是数字类型
    var currentAngle = Number(this.angle);
    // 限制步进值为30，最小不低于0
    ...
    // 使用新的快速更新方法
    this.updateCurtainAngle(newAngle);
  }), (0, _defineProperty2.default)(_methods, "updateCurtainAngle", function updateCurtainAngle(angle) {
    var _this9 = this;
    // 确保参数是数字
    angle = Number(angle);
    if (isNaN(angle)) {
    ...
    this.sendPropertyUpdate({
      ...
      _this9.verifyAngleChanged(initialAngle);
    }).catch(function (err) {
      console.log('角度调整命令发送失败，但可能仍在执行:', err);
      // 即使API失败也进行验证
      _this9.verifyAngleChanged(initialAngle);
    });
  }), (0, _defineProperty2.default)(_methods, "verifyAngleChanged", function verifyAngleChanged(initialAngle) {
    var _this10 = this;
    // 设置一个短的延迟后验证
    setTimeout(function () {
      // 直接获取最新数据，不显示loading
      _this10.getDataFromOnenet(true).then(function () {
        // 检查实际角度与显示角度的差异，如果太大则可能需要修正
        if (Math.abs(_this10.angle - _this10.targetAngle) > 15) {
        ...
        // 不论结果如何，清理状态
        _this10.targetAngle = null;
      }).catch(function () {
        // 错误时也清理状态，保持静默
        _this10.targetAngle = null;
      });
    }, 1500); // 延迟1.5秒后检查，给设备足够响应时间
  }), (0, _defineProperty2.default)(_methods, "switchMode", function switchMode(newMode) {
    var _this11 = this;
    ...
    this.sendPropertyUpdate({
      ...
      // 5秒后自动清除切换状态
      setTimeout(function () {
        _this11.isSwitchingMode = false;
        _this11.targetMode = null;
      }, 5000);
    });
  }), (0, _defineProperty2.default)(_methods, "executeModeSwitch", function executeModeSwitch(newMode) {
    var _this12 = this;
    this.sendPropertyUpdate({
      "mode": newMode
      ...
      // 立即关闭loading并显示成功提示
      uni.hideLoading();
      _this12.isSwitchingMode = false;
      _this12.currentDeviceMode = newMode;
      ...
      // 失败时也立即关闭loading提示
      uni.hideLoading();
      if (_this12.modeSwitchRetries < 2) {
        _this12.modeSwitchRetries++;
        console.log("\u6A21\u5F0F\u5207\u6362\u91CD\u8BD5\u7B2C".concat(_this12.modeSwitchRetries, "\u6B21..."));

        // 静默重试，不再显示loading
        _this12.switchModeTimer = setTimeout(function () {
          _this12.executeModeSwitch(newMode);
        }, 300);
      } else {
        // 最后一次重试也失败，结束切换状态
        _this12.isSwitchingMode = false;
        ...
        setTimeout(function () {
          return _this12.verifyModeStatus(newMode);
        }, 3000);
      }
    });
  }), (0, _defineProperty2.default)(_methods, "verifyModeStatus", function verifyModeStatus(expectedMode) {
    // 如果当前不是处于期望的模式，主动获取一次数据
    if (this.mode !== this.currentDeviceMode) {
      this.getDataFromOnenet(true);
    }
  }), (0, _defineProperty2.default)(_methods, "onStartTimeChange", function onStartTimeChange(e) {
  ...
  }), (0, _defineProperty2.default)(_methods, "onTimerAngleChange", function onTimerAngleChange(e) {
  ...
  }), (0, _defineProperty2.default)(_methods, "debounceSave", function debounceSave(property, value) {
    var _this13 = this;
    // 取消之前的定时器
    if (this.saveTimer) {
    ...
    this.saveTimer = setTimeout(function () {
      _this13.saveProperty(property, value);
    }, 800);
  }), (0, _defineProperty2.default)(_methods, "saveProperty", function saveProperty(property, value) {
    var _this14 = this;
    // 避免重复保存同一属性
    ...
    this.sendOneProperty(property, value).then(function () {
      ...
      _this14.savingProperty = '';
      // 保存完成后2秒钟再恢复同步
      setTimeout(function () {
        _this14.isEditingTime = false;
      }, 2000);
    });
  }), (0, _defineProperty2.default)(_methods, "sendOneProperty", function sendOneProperty(key, value) {
    var _this15 = this;
    // 构造单属性参数对象
    var params = {};
    params[key] = value;
    ...
    return new Promise(function (resolve) {
      ...
      _this15.sendPropertyUpdate(params, true).then(function (res) {
